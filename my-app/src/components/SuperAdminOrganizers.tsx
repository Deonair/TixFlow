...
export default function SuperAdminOrganizers() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const orgRes = await fetch('/api/admin/organizers?includeStats=true', { credentials: 'include' })
        if (!orgRes.ok) {
  ...
  const login = async () => {
    ...
    try {
      ...
      setError(null)
      try {
        const res2 = await fetch('/api/admin/organizers?includeStats=true', { credentials: 'include' })
        if (!res2.ok) {
  ...
  if (needsLogin) {
    return (
      <div className="mx-auto max-w-md">
        <h1 className="text-2xl font-semibold text-gray-900">SuperAdmin — Organizers</h1>
        <p className="mt-1 text-sm text-gray-600">Log in om de lijst van organisaties te beheren.</p>
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <h1 className="text-2xl font-semibold text-gray-900">SuperAdmin — Organizers</h1>
      <p className="mt-1 text-sm text-gray-600">Beheer organisaties en financiële gegevens.</p>

      <div className="mt-6 overflow-x-auto overflow-y-auto max-h-96 rounded-xl border border-gray-200">
